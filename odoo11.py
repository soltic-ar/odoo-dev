...
# Based on github.com/odoo/odoo/odoo11.py.
#
# wget https://raw.githubusercontent.com/soltic-ar/odoo-dev/master/odoo11.py
# 
#----------------------------------------------------------
import os
import sys
...
def cmd_server(args=[]):
    ...
    try:
        if os.path.isfile('./odoo/odoo11.py'):
            run(['./odoo/odoo11.py', 'server', '--config=%s'%get_current()] + args)
...
def cmd_scaffold(args=[]):
    try:
        if os.path.isfile('./odoo/odoo11.py'):
            run(['./odoo/odoo11.py', 'scaffold'] + args)
    except:
        pass    

def cmd_help(args=[]):
    print ("usage: odoo11.py <command> [<args>]")
    print ("Commands:")
    print ("   environment:")
